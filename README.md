...
LLM 視需要連續呼叫工具（最多 8 步）：
  ...
  check_conventions / find_related_tables / check_table_docs / draft_comment_ddl
     ↓
每次工具結果都截斷後回饋給 LLM 繼續推理
     ↓
LLM 輸出 <FINAL> 結束本回合，回覆可能包含：
  - 一般文字說明
  - <DDL_SUGGESTION>（建議的結構變更，使用者按「執行 DDL」確認後才會執行）
...
- 查詢與 DDL 建議仍遵循既有安全層（`web/sql_safety.py`）：`run_query` 僅接受單一 SELECT/EXPLAIN；DDL 建議需人工按鈕確認才會經 allowlist 檢查後執行
...
│   ├── tool_registry.py         # DB Agent 可呼叫的工具目錄（list_databases / get_schema / run_query / ...）
│   └── writers/
