...
def check_conventions(design_tables: list[TableSpec], conventions: dict) -> list[dict]:
    ...
    for t in design_tables:
        if naming_confident and _style(t.table_name) != naming_style:
            warnings.append({
                "level": "warn", "code": "convention_naming", "table": t.table_name,
                "column": "",
                "message": f"表名為 {_style(t.table_name)} 風格，"
                           f"與現有資料庫多數採用的 {naming_style} 命名不一致",
            })

        col_names = {c.name.lower() for c in t.columns}

        if (conventions.get("timestamp_ratio", 0) >= _RATIO_THRESHOLD
                and "created_at" not in col_names):
            warnings.append({
                "level": "info", "code": "convention_timestamps", "table": t.table_name,
                "column": "",
                "message": "現有資料庫多數資料表都有 created_at 稽核欄位，建議補上",
            })

        if (conventions.get("soft_delete_ratio", 0) >= _RATIO_THRESHOLD
                and not (col_names & _SOFT_DELETE_COLS)):
            warnings.append({
                "level": "info", "code": "convention_soft_delete", "table": t.table_name,
                "column": "",
        ...
        for c in t.columns:
            if naming_confident and _style(c.name) != naming_style:
                warnings.append({
                    "level": "warn", "code": "convention_naming", "table": t.table_name,
                    "column": c.name,
                    "message": f"欄位名為 {_style(c.name)} 風格，"
                               f"與現有資料庫多數採用的 {naming_style} 命名不一致",
                })
            ...
            if (c.is_primary_key and conventions.get("pk_type")
                ...
                warnings.append({
                    "level": "info", "code": "convention_pk_type", "table": t.table_name,
                    "column": c.name,
                    "message": f"主鍵型態為 {c.data_type}，"
                               f"與現有資料庫多數採用的 {conventions['pk_type']} 不一致",
                })

            if (c.is_foreign_key and conventions.get("fk_suffix_ratio", 0) >= _FK_SUFFIX_THRESHOLD
                    and not c.name.lower().endswith("_id")):
                warnings.append({
                    "level": "info", "code": "convention_fk_naming", "table": t.table_name,
                    "column": c.name,
