...
def analyze(tables: list[TableSpec]) -> list[dict]:
    ...
    for t in tables:
        ...
        if not any(c.is_primary_key for c in cols):
            warnings.append({
                "level": "warn", "code": "no_pk", "table": t.table_name, "column": "",
        ...
        if not (col_names & set(_AUDIT_COLS)):
            warnings.append({
                "level": "info", "code": "missing_audit", "table": t.table_name, "column": "",
        ...
        for c in cols:
            ...
            if c.is_foreign_key and not c.is_indexed and not c.is_primary_key:
                warnings.append({
                    "level": "warn", "code": "fk_no_index", "table": t.table_name, "column": c.name,
            ...
            if any(h in name_l for h in _UNIQUE_HINTS) and not c.is_unique and not c.is_primary_key:
                warnings.append({
                    "level": "warn", "code": "likely_unique", "table": t.table_name,
                    "column": c.name, "message": "看起來是業務唯一值，建議加上 UNIQUE 約束",
                })

            # 5. Secret field — must be hashed/encrypted
            if any(h in name_l for h in _SECRET_HINTS):
                warnings.append({
                    "level": "warn", "code": "secret_plaintext", "table": t.table_name,
                    "column": c.name, "message": "疑似敏感資料，務必雜湊或加密儲存，切勿明文",
                })

            # 6. varchar without length
            if type_l in ("varchar", "character varying") and not c.length:
                warnings.append({
                    "level": "info", "code": "varchar_no_length", "table": t.table_name,
                    "column": c.name, "message": "varchar 未指定長度，建議明確設定上限",
                })

            # 7. enum-like field without CHECK constraint
            if any(h in name_l for h in _ENUM_HINTS) and type_l in (
                ...
                if not has_check:
                    warnings.append({
                        "level": "info", "code": "enum_no_check", "table": t.table_name,
                        "column": c.name, "message": "狀態/類型欄位建議加 CHECK 約束限制可用值",
                    })

            # 8. plain timestamp (no timezone)
            if type_l == "timestamp":
                warnings.append({
                    "level": "info", "code": "naive_timestamp", "table": t.table_name,
                    "column": c.name,
            ...
            if re.search(r"[a-z][A-Z]", c.name):
                warnings.append({
                    "level": "warn", "table": t.table_name, "column": c.name,
                    "message": "使用 camelCase 命名，PostgreSQL 慣例為 underscore_case，"
                               "否則查詢需加引號",
                })
            ...
            if name_l in _BLOB_HINTS and type_l in ("json", "jsonb", "text"):
                warnings.append({
                    "level": "info", "table": t.table_name, "column": c.name,
                    "message": "泛用欄位（如 data/metadata）建議拆成具名欄位，較易查詢與索引",
                })

        # 11. entity table with audit fields but no soft-delete column
        if (
            (col_names & set(_AUDIT_COLS))
            and not (col_names & set(_SOFT_DELETE_COLS))
            and len(cols) >= 5
        ):
            warnings.append({
                "level": "info", "table": t.table_name, "column": "",
