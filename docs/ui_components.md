...
用途：`base.html` 頂欄使用者選單。頁面載入時打 `GET /api/v1/auth/me`，
`anonymous: true` 或請求失敗（含 401、後端 `/auth/me` 尚未合併的 404）時
整個容器維持 `hidden`，匿名模式（`AUTH_ENABLED=false`）畫面不變。
子元件：`.user-menu-trigger`（顯示 display_name/email 的觸發鈕，
`data-action="toggle-user-menu"`）、`.user-menu-dropdown`（展開面板，
內含 `.user-menu-email` 與 `data-action="logout"` 登出／切換使用者鈕）。
Token 依賴：`--color-bg-panel`、`--color-text-400/700/900`、`--border-color`、
`--radius-sm/md`、`--z-dropdown`、`--space-*`。
...
`/login`（`login.html`）不套用 `base.html`（無側欄／頂欄，避免未登入使用者看到
站內導覽），採獨立版面 `.page-login-shell`（置中）+ `.page-login-card`
（重用 `.card`）。子元件：`.page-login-logo`、`.page-login-error`（401/429/
其他錯誤訊息文字）、`.page-login-divider` 與 `.page-login-sso`（SSO 按鈕，
`static/js/login.js` 探測 `GET /api/v1/auth/sso` 存在才取消 `hidden`）。
表單欄位重用既有 `.form-field`／`.form-input`／`.btn-primary`。
