...
def _parse_column(line: str) -> ColumnSpec | None:
    ...
    default_m = re.search(r'DEFAULT\s+([^,]+?)(?:\s+(?:NOT\s+NULL|UNIQUE|PRIMARY|REFERENCES|CHECK)|$)', rest, re.IGNORECASE)
    ...
    for part in body_parts:
        ...
        if fk_m:
            ...
            if cname in cols:
                ...
                cols[cname].references = fk_m.group(2) + (f".{fk_m.group(3)}" if fk_m.group(3) else "")
                cols[cname].is_indexed = True
