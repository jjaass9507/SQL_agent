...
def _hidden_table_names() -> set[str]:
    """Reuse the platform's own bookkeeping-table list (sessions/messages/
    activity_log/alembic_version) so they never show up as "missing docs"."""
    from web.db_schema import platform_table_names
    return {n.lower() for n in platform_table_names()}


def check_metadata_completeness(existing_tables: list[TableSpec]) -> dict:
    """Return {"summary": {...}, "missing": [...]} describing how much of the
    existing DB has table/column comments, excluding platform tables."""
    hidden = _hidden_table_names()
    ...
    covered_items = (tables_total - tables_without_comment) + (columns_total - columns_without_comment)
...
def draft_comment_ddl(db: str, table: str, comments: dict) -> str:
    ...
    if table_comment:
        statements.append(f"COMMENT ON TABLE {qualified_table} IS '{_escape_literal(table_comment)}';")

    for col_name, comment in ((comments or {}).get("columns") or {}).items():
        if not comment:
            continue
        statements.append(
            f"COMMENT ON COLUMN {qualified_table}.{_quote_ident(col_name)} IS '{_escape_literal(comment)}';"
        )
