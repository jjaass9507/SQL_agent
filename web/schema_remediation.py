...
def _runnable_sql(code: str, t: str, c: str) -> str | None:
    ...
    if code == "missing_audit":
        return (f"ALTER TABLE {t} ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();\n"
...
def _todo_comment(code: str, t: str, c: str) -> str | None:
    ...
    if code == "varchar_no_length":
        return f"-- [{t}.{c}] varchar 未指定長度：評估上限後 ALTER TABLE {t} ALTER COLUMN {c} TYPE varchar(<N>);"
    if code == "enum_no_check":
        return f"-- [{t}.{c}] 建議限制可用值：ALTER TABLE {t} ADD CONSTRAINT chk_{t}_{c} CHECK ({c} IN ('<v1>','<v2>'));"
...
def build_remediation_sql(warnings: list[dict]) -> str:
    ...
    todos: list[str] = []
    for w in warnings:
        code, t, c, msg = w.get("code", ""), w.get("table", ""), w.get("column", ""), w.get("message", "")
        if not t:
            continue
